...
function checkUserPermissions($id_surat)
{
    ...
    $query = mysqli_query($config, "SELECT id_user FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
...
function handleFileUploadAndUpdate($data)
{
    ...
    // Update database
    if (updateSuratMasuk($data, $fileName)) {
...
/**
 * Update surat_masuk table
 */
function updateSuratMasuk($data, $fileName = null)
{
    global $config;
    ...
    $updateFields = [
        ...
        "asal_surat = '{$escapedData['asal_surat']}'",
        "isi = '{$escapedData['isi']}'",
        "kode = '{$escapedData['kode']}'",
        "indeks = '{$escapedData['indeks']}'",
    ...
    $query = "UPDATE tbl_surat_masuk SET " . implode(', ', $updateFields) .
...
function getSuratData($id_surat)
{
    ...
    $query = mysqli_query(
        $config,
        "SELECT id_surat, no_agenda, no_surat, asal_surat, isi, kode, indeks, 
                ...
                <div class="nav-wrapper blue-grey darken-1">
                    <ul class="left">
                        <li class="waves-effect waves-light">
                            <a href="#" class="judul">
                                <i class="material-icons">edit</i> Edit Data Surat Masuk
                            </a>
                        </li>
                    </ul>
                </div>
...
function displayFormFields($suratData)
{
    ...
    <div class="row">
        <?php
        displayHiddenIdField($suratData['id_surat']);
        displayNumberAgendaField($suratData['no_agenda']);
        displayKodeField($suratData['kode']);
        displayAsalSuratField($suratData['asal_surat']);
        displayIndeksField($suratData['indeks']);
...
function displayHiddenIdField($value)
{
    echo '<input type="hidden" name="id_surat" value="' . htmlspecialchars($value) . '">';
}

function displayNumberAgendaField($value)
{
    displayFormField('number', 'no_agenda', '', 'looks_one', $value, 'eno_agenda');
}

function displayKodeField($value)
{
    displayFormField('text', 'kode', '', 'bookmark', $value, 'ekode');
}

function displayAsalSuratField($value)
{
    displayFormField('text', 'asal_surat', '', 'place', $value, 'easal_surat');
}

function displayIndeksField($value)
{
    displayFormField('text', 'indeks', '', 'storage', $value, 'eindeks');
}

function displayNoSuratField($value)
{
    displayFormField('text', 'no_surat', '', 'looks_two', $value, 'eno_surat');
}
...
function displayTanggalSuratField($value)
{
    ...
    if (isset($_SESSION['etgl_surat'])) {
        echo '<div id="alert-message" class="callout bottom z-depth-1 red lighten-4 red-text">' .
            htmlspecialchars($_SESSION['etgl_surat']) . '</div>';
...
{
    displayFormField('textarea', 'isi', '', 'description', $value, 'eisi');
}

function displayKeteranganField($value)
{
    displayFormField('text', 'keterangan', '', 'featured_play_list', $value, 'eketerangan');
}
